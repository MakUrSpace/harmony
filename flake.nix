        ...
        packages.harmony-ngrok = writeShellApplication {
          ...
          text = ''
            ...
            tmux split-window -t "$SESSION_NAME:0" -h \
              "echo 'Exposing Harmony on $HARMONY_URL -> $PORT'; ngrok http $PORT --domain=$HARMONY_URL --upstream-protocol=http2; read"
            
            # Split for Admin Ngrok (vertical split of the new pane)
            tmux split-window -t "$SESSION_NAME:0.1" -v \
              "echo 'Exposing Admin on $ADMIN_URL -> $ADMIN_PORT'; ngrok http $ADMIN_PORT --domain=$ADMIN_URL --upstream-protocol=http2; read"
