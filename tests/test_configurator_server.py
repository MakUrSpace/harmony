...
def test_grid_configuration(configurator_client, configurator_app):
    ...
    response = configurator_client.post(
        "/configurator/grid_configuration", data={"size": "25"}
    )
    assert response.status_code == 200
...
def test_manual_calibration_logic(configurator_client, configurator_app):
    ...
    with (
        mock.patch.object(
            configurator_app.state.cm, "buildRealSpaceConverter"
        ) as mock_build,
        mock.patch.object(
            configurator_app.state.cc, "axial_to_pixel", return_value=(50.0, 50.0)
        ),
    ):
        response = configurator_client.post(
            "/configurator/manual_calibration", json=data
        )
...
def test_nudge_select(configurator_client, configurator_app):
    ...
    response = configurator_client.get(
        f"/configurator/nudge_select/{cam_name}?px=0.5&py=0.5"
    )
...
def test_nudge_hex_apply(configurator_client, configurator_app):
    ...
    if not hasattr(configurator_app.state.cc.hex, "hex_nudges") or not isinstance(
        configurator_app.state.cc.hex.hex_nudges, dict
    ):
...
def test_nudge_hex_reset(configurator_client, configurator_app):
    ...
    payload = {"hexes": [{"q": 2, "r": 3}], "reset": True}
