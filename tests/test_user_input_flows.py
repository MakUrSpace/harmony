...
class TestSaveLoad:
    ...
    def test_load_game_existing(self, harmony_client):
        ...
        with tempfile.NamedTemporaryFile(
                suffix='.pickle', delete=False,
                dir=os.getcwd(), prefix='testgame_') as f:
...
class TestSessionManagement:
    def test_update_session_id_rename(self, harmony_client):
        ...
        r = harmony_client.post('/harmony/update_session_id',
...
class TestObjectCRUD:
    def _inject_obj(self, hs, oid):
        obj = _mock_obj(oid)
        hs._cm.memory = [obj]
        hs._cm.findObject.side_effect = lambda objectId: obj if objectId == oid else None
        return obj
    ...
    def test_get_object_found(self, harmony_client):
        ...
        with mock.patch('harmony.harmonyServer.imageToBase64', return_value='b64data'), \
             mock.patch('harmony.harmonyServer.buildObjectSettings', return_value='settings_html'):
    ...
    def test_delete_object_admin(self, harmony_client):
        ...
        r = harmony_client.request('DELETE', '/harmony/objects/Target',
                                   cookies={'session_view_id': view_id})
        assert r.status_code == 200
    ...
    def test_post_object_rename(self, harmony_app):
        ...
        mock_cm.findObject.side_effect = lambda objectId=None: obj if objectId == 'OldName' else None
        ...
        with mock.patch('harmony.harmonyServer.imageToBase64', return_value='b64'), \
             mock.patch('harmony.harmonyServer.buildObjectTable', return_value='<p>ok</p>'):
    ...
    def test_post_object_rename_duplicate(self, harmony_app):
        ...
        def find(objectId=None):
            if objectId == 'OrigObj2': return obj
            if objectId == 'DupeObj2': return existing
            return None
...
class TestObjectFactory:
    ...
    def test_post_object_factory_single(self, harmony_client):
        ...
        hs._cm.cc = hs._cc
        r = harmony_client.post(f'/harmony/object_factory/{view_id}',
                                data={'object_name': 'Widget'})
        assert r.status_code == 200
...
class TestMoveRequest:
    ...
    def test_move_object_not_moveable(self, harmony_client):
        ...
        hs._cm.findObject.side_effect = lambda oid: obj if oid == 'ImmovableObj' else None
...
class TestVirtualMapSelection:
    def test_select_pixel_virtual_map(self, harmony_client):
        ...
        view_id = 'vm_sel'
        _setup_session(hs, view_id)
        r = harmony_client.post('/harmony/select_pixel', data={
            'viewId': view_id,
...
class TestConfiguratorInputFlows:
    def test_nudge_commit_single_cell(self, configurator_client, configurator_app):
        ...
        payload = {'hexes': [{'q': 1, 'r': 2}], 'dx': 8, 'dy': -4,
                   'reset': False, 'reset_all': False}
        r = configurator_client.post('/configurator/nudge/Camera 0',
                                     json=payload)
        assert r.status_code == 200
        data = r.json()
    ...
    def test_set_overlays(self, configurator_client, configurator_app):
        r = configurator_client.post('/configurator/set_overlays',
    ...
    def test_grid_config_updates_hex(self, configurator_client, configurator_app):
        r = configurator_client.post('/configurator/grid_configuration',
                                     data={'size': '30'})
        assert r.status_code == 200
