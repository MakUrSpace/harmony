...
sys.path.insert(
    0, os.path.abspath(os.path.join(os.path.dirname(__file__), "../observer"))
)
sys.path.insert(
    0, os.path.abspath(os.path.join(os.path.dirname(__file__), "../harmony"))
)
...
def mock_cv2():
    ...
    mock_cv2_module.imencode.return_value = (
        True,
...
def observer_app(mock_cv2):
    ...
    with mock.patch("observer.calibrator.CalibratedCaptureConfiguration") as MockCC:
        ...
        mock_cc.cameras["Camera 0"].mostRecentFrame = np.zeros(
            (480, 640, 3), dtype=np.uint8
        )
        mock_cc.cameras["Camera 0"].cropToActiveZone.return_value = np.zeros(
            (480, 640, 3), dtype=np.uint8
        )
...
def configurator_app(mock_cv2):
    ...
    with mock.patch(
        ...
        mock_cc.cameras["Camera 0"].activeZone = np.array(
            [[0, 0], [0, 1], [1, 1], [1, 0]], dtype=np.float32
        )
...
def harmony_app(mock_cv2):
    ...
    with (
        mock.patch("observer.calibrator.CalibratedCaptureConfiguration") as MockCC,
        ...
        mock_hex_cc.cameras["Camera 0"].mostRecentFrame = np.zeros(
            (480, 640, 3), dtype=np.uint8
        )
