...
def renderConsole():
    while True:
        ...
        mode = f"Mode: {app.cm.mode:7}" + (
            "" if app.cm.dowel_position is None else f"-{app.cm.dowel_position}"
        )
        consoleImage = cv2.putText(
...
def controlSetTrack(position):
    assert position in ["first", "top", "hypos", "longs", "shorts"], (
        f"Unrecognzed dowel position: {position}"
    )
    with DATA_LOCK:
...
def buildModeController():
    return (
        """
            ...
            </div>""".replace("{calibratorURL}", url_for(".buildCalibrator"))
        .replace("{passiveChecked}", 'checked=""' if app.cm.mode == "passive" else "")
        .replace(
            "{firstChecked}",
            'checked=""'
            if app.cm.mode == "track" and app.cm.dowel_position == "first"
            else "",
        )
        .replace(
            "{topChecked}",
            'checked=""'
            if app.cm.mode == "track" and app.cm.dowel_position == "top"
            else "",
        )
        .replace(
            "{hyposChecked}",
            'checked=""'
            if app.cm.mode == "track" and app.cm.dowel_position == "hypos"
            else "",
        )
        .replace(
            "{longsChecked}",
            'checked=""'
            if app.cm.mode == "track" and app.cm.dowel_position == "longs"
            else "",
        )
        .replace(
            "{shortsChecked}",
            'checked=""'
            if app.cm.mode == "track" and app.cm.dowel_position == "shorts"
            else "",
        )
    )
...
def buildCalibrator():
    ...
    return (
        template.replace("{cameraGrid}", cameraGrid)
        .replace("{cameraNamesJson}", cameraNamesJson)
        .replace("{calibratorURL}", url_for(".buildCalibrator"))
    )
