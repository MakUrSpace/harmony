...
def numpyizeDeserializedCameraChange(deserializedCameraChange: dict):
    if deserializedCameraChange["camName"] is not None:
        deserializedCameraChange["changeContours"] = [
            np.array(contour, dtype=np.int32)
            for contour in deserializedCameraChange["changeContours"]
        ]
        deserializedCameraChange["after"] = np.array(
            deserializedCameraChange["after"], dtype=np.int32
        )
        deserializedCameraChange["before"] = np.array(
            deserializedCameraChange["before"], dtype=np.int32
        )
        deserializedCameraChange["lastChange"] = numpyizeDeserializedCameraChange(
            deserializedCameraChange["lastChange"]
        )
...
class HarmonyObject(TrackedObject):
    ...
    objectFactories = {
        "Unit": mc.MechFactories,
        "Structure": mc.StructureFactories,
        "Terrain": mc.TerrainFactories,
    }

    def __post_init__(self):
        super().__post_init__()
        print(
            f"Creating {self.objectType} -- {self.objectSubType} -- {self.objectKwargs}"
        )
        self.objectKwargs = {
            key.lower(): value for key, value in self.objectKwargs.items()
        }
    ...
    def construct(self):
        try:
            ...
            raise Exception(
                f"Unrecognized object type and subtype - {self.objectType}-{self.objectSubType}"
            ) from ke
        try:
...
class ObjectAction:
    ...
    def __post_init__(self):
        ...
        self.targetRange = (
            "short"
            if self.targetDistance < MEDIUM_RANGE
            else "medium"
            if self.targetDistance < LONG_RANGE
            else "long"
        )
        ...
        self.tMM = (
            0
            if self.targetMovement is None or self.targetMovement < 10
            else mc.TMM(self.actor.oid).terminant
        )
    ...
    def declare(self):
        self.gameEvent = mc.GameEvent.declare_event(
            "Attack",
            mc.Faction(self.actor.oid).terminant,
            self.actor.oid,
            self.target.oid,
            self.targetNumber,
            self.result,
...
class HarmonyDB(mc.Graph):
    ...
    def memorize(self, hObj: HarmonyObject):
        ...
        with self.DB_LOCK, self.cursor() as cur:
            ...
            cur.execute(
                f"INSERT INTO harmony_objects VALUES ('{hObj.oid}', '{serializedHarmonyObject}')"
            )
    ...
    def rename(self, entity, newEntity):
        ...
        with self.DB_LOCK, self.cursor() as cur:
            mem = list(
                cur.execute(
                    f"SELECT * FROM harmony_objects WHERE oid='{entity}'"
                ).fetchall()[0]
            )
            mem[0] = newEntity
...
class HarmonyMachine(CalibratedObserver):
    ...
    def classifyObject(
        self, objectId: str, objectType: str, objectSubType: str, objectKwargs: dict
    ):
        ...
        mc.XYLocation.set_relationship(
            harmonyObject.oid, self.cc.rsc.changeSetToRealCenter(harmonyObject)
        )
        mc.Footprint.set_relationship(
            harmonyObject.oid, self.cc.objectToHull(harmonyObject)
        )
    ...
    def captureRealCoord(self, capture):
        ...
        return [
            *self.cc.rsc.changeSetToRealCenter(capture),
            self.objectElevation(capture),
        ]

    def captureSystemCoord(self, capture):
        return [
            *mc.XYLocation(capture.oid).terminant,
            mc.Elevation(capture.oid).terminant,
        ]
    ...
    def commitChanges(self, objDef, overwrite=True):
        try:
            ...
            if (
                existingMem.expectedChange is not None
                and existingMem == existingMem.expectedChange.lastChange
        ...
        if isinstance(objDef, HarmonyObject):
            ...
            self.declareEvent(
                ...
                eventResult=json.dumps(
                    self.cc.objectToHull(objDef), default=numpy_serializer
                ),
            )
    ...
    def objectLocation(self, harmonyObject) -> tuple[float, float, float]:
        if mc.GameState.getPhase() == "Move":
            ...
            return [
                *mc.XYLocation(harmonyObject.oid).terminant,
                mc.Elevation(harmonyObject.oid).terminant,
            ]

    def distanceFromRealPoint(
        self, harmonyObject: HarmonyObject, realPoint: (float, float, float)
    ) -> float:
        ...
        distance = (
            distanceFormula(objectPoint[:2], realPoint[:2])
            + abs(realPoint[2] - objectPoint[2]) * 2
        ) * MM_TO_INCHES
        return distance
    ...
    def objectLastDistance(self, harmonyObject: HarmonyObject):
        if mc.GameState.getPhase() == "Move":
            ...
            delta = [
                d1 - d0
                for d0, d1 in zip(
                    mc.XYLocation(harmonyObject.oid).terminant, currentPoint
                )
            ]
            return mc.Movement.calculate_movement(
                entity=harmonyObject.oid, delta=delta
            )[0]
        else:
    ...
    def line_of_sight(
        self, actor: HarmonyObject, target: HarmonyObject, step_size: int = 5
    ):
        actorPersp = [
            *mc.XYLocation(actor.oid).terminant,
            mc.Elevation(actor.oid).terminant,
        ]
        targetPersp = [
            *mc.XYLocation(target.oid).terminant,
            mc.Elevation(target.oid).terminant,
        ]
        ...
        distanceBetween = (
            sum([(d1 - d0) ** 2 for d0, d1 in zip(actorPersp, targetPersp)]) ** 0.5
        )
        visualObstacles = []
        print(
            f"== LOS Between {actor.oid} and {target.oid} ({elevationChange}) [{actorPersp} to {targetPersp}]"
        )
        for m in self.memory:
            ...
            distanceFromHigher = (
                sum([(d1 - d0) ** 2 for d0, d1 in zip(higherEntity[:2], memXY)]) ** 0.5
            )
            if distanceFromHigher > distanceBetween:
                continue

            losElevationAtDistance = (
                higherEntity[2]
                - (distanceFromHigher / distanceBetween) * elevationChange
            )
            memElevation = mc.Elevation(m.oid).terminant
            print(
                f"=== Evaluating {m.oid} -- LOSElev: {losElevationAtDistance} || memElev: {memElevation}"
            )
            if memElevation > losElevationAtDistance:
                visualObstacles.append(m)

        print(
            f"== LOS Between {actor.oid} and {target.oid}: {[m.oid for m in visualObstacles]}"
        )
    ...
    def can_target(self, actor: HarmonyObject, target: HarmonyObject):
        return (
            target.oid != actor.oid
            and self.object_type(target.oid) in ["Unit", "Structure"]
            and self.faction(actor.oid) != self.faction(target.oid)
            and self.line_of_sight(actor, target)
            and not self.obj_destroyed(target.oid)
        )
    ...
    def objectsAndColors(self):
        ...
        factionColors = {
            faction: color
            for faction, color in zip(factions, self.distinct_colors(len(factions)))
        }
        ...
        for mem in self.memory:
            try:
                ...
                unknown.append(mem)
        return [
            MiniMapObject(mem, factionColors[self.faction(mem.oid)])
    ...
    def objectElevation(self, obj):
        if mc.GameState.getPhase() == "Move":
            shift_xy = [
                d0 - d1
                for d0, d1 in zip(
                    self.cc.rsc.changeSetToRealCenter(obj),
                    mc.XYLocation(obj.oid).terminant,
                )
            ]
        else:
            shift_xy = (0, 0)
        ...
        return mc.Movement.shifted_location_elevation_difficulty(
            entity=obj.oid, x=shift_xy[0], y=shift_xy[1], footprint=footprint
        )[0]
                    ...
                    miniMap = cv2.circle(
                        ...
                        np.array(
                            [
                                d - dd
                                for d, dd in zip(
                                    self.cc.rsc.changeSetToRealCenter(
                                        memObj.expectedChange
                                    ),
                                    [x, y],
                                )
                            ],
                    ...
                    miniMap = cv2.line(
                        ...
                        np.array(
                            [
                                ...
                                for d, dd in zip(
                                    self.cc.rsc.changeSetToRealCenter(
                                        memObj.expectedChange.lastChange
                                    ),
                                    [x, y],
                                )
                            ],
    ...
    def walkDirection(self, harmonyObject, direction: tuple[float, float], step_size=5):
        ...
        step = 1
        movement_cost, elevation, movements = mc.Movement.calculate_movement(
            entity=harmonyObject.oid, delta=[dx, dy], max_cost=movement_speed
        )
    ...
    def buildMovementHeatMap(self, harmonyObject: HarmonyObject):
        ...
        movementHeatMap = getattr(harmonyObject, "movementHeatMap", {}).get(
            str(mc.XYLocation(harmonyObject.oid).terminant), None
        )
        if movementHeatMap is None:
            ...
            for dx, dy in generate_ds():
                (
                    ...
                    path_long_moves,
                    path_full_moves,
                ) = self.walkDirection(harmonyObject, [dx, dy])
            ...
            layer_hulls = []
            for moves, color in zip(
                [near_moves, med_moves, long_moves, full_moves],
                [(0, 255, 0), (0, 180, 125), (0, 125, 255), (0, 0, 255)],
            ):
                if len(moves) == 0:
                    continue
            ...
            harmonyObject.movementHeatMap = {
                str(mc.XYLocation(harmonyObject.oid).terminant): image
            }
        else:
        ...
        minimap = self.buildMiniMap(
            objectsAndColors=[
                MiniMapObject(
                    ...
                    if self.object_type(mem.oid) == "Unit"
                    else (255, 0, 0),
                )
                for mem in self.memory
            ],
    ...
    def buildVisibilityMap(self, harmonyObject: HarmonyObject):
        ...
        visibilityMap = getattr(harmonyObject, "visibilityMap", {}).get(
            str(self.objectLocation(harmonyObject)), None
        )
        if visibilityMap is None:
            ...
            for radius, color in zip(
                [42 * INCHES_TO_MM, 24 * INCHES_TO_MM, 6 * INCHES_TO_MM],
                [(0, 0, 255), (0, 125, 255), (0, 255, 0)],
            ):
            ...
            for target in self.memory:
                if self.can_target(harmonyObject, target):
                    cv2.line(
                        image,
                        center,
                        [int(d) for d in self.objectLocation(harmonyObject)[:2]],
                        color=(0, 0, 0),
                        thickness=2,
                    )

            image = image[y : y + h, x : x + w]
            harmonyObject.visibilityMap = {
                str(self.objectLocation(harmonyObject)): image
            }
        else:
            image = visibilityMap
        ...
        minimap = self.buildMiniMap(
            objectsAndColors=[
                MiniMapObject(mem, select_color(mem)) for mem in self.memory
            ],
            showInteractions=False,
        )
    ...
    def add_footprint(
        ...
        hObj: HarmonyObject,
        camName: str,
        addition_points: list[tuple[int, int]],
        margin=50,
    ):
        cam = self.cc.cameras[camName]
        change = hObj.changeSet[camName]
        x, y, w, h = hObj.changeSet["0"].clipBox
        adjusted_addition_points = [
            [pt[0] + x - margin, pt[1] + y - margin] for pt in addition_points
        ]
        ...
        contour_image = cv2.drawContours(
            blank_image.copy(), change.changeContours, -1, (255), -1
        )
        projected_addition_and_contours = cv2.polylines(
            contour_image,
            [adjusted_addition_points],
            isClosed=True,
            color=(255),
        ...
        newChangeContours = cv2.findContours(
            thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )[0]
        hObj.changeSet[camName].overrideChangeContours(newChangeContours)
        return hObj

    def subtract_footprint(
        self,
        hObj: HarmonyObject,
        camName: str,
        subtraction_points: list[tuple[int, int]],
        margin=50,
    ):
        cam = self.cc.cameras[camName]
        change = hObj.changeSet[camName]
        ...
        x, y, w, h = hObj.changeSet["0"].clipBox
        adjusted_subtraction_points = [
            [pt[0] + x - margin, pt[1] + y - margin] for pt in subtraction_points
        ]
        adjusted_subtraction_points = np.int32(adjusted_subtraction_points)
        height, width = cam.mostRecentFrame.shape[:2]
        mask = np.ones([height, width], dtype=np.uint8) * 255  # White mask
        contour_image = cv2.drawContours(
            mask.copy(), [adjusted_subtraction_points], -1, (0), -1
        )
        print(f"Created contour image")

        existing_contours = cv2.drawContours(
            np.zeros([height, width], np.uint8), change.changeContours, -1, (255), -1
        )
        projected_subtraction_and_contours = cv2.bitwise_and(
            existing_contours, existing_contours, mask=contour_image
        )
...
if __name__ == "__main__":
    ...
    hm.classifyObject(
        hm.memory[0].oid,
        objectType="Unit",
        objectSubType="Kingfisher Prime",
        objectKwargs={"Faction": "Foo", "Skill": 4},
    )
    ...
    hm.classifyObject(
        ...
        objectType="Unit",
        objectSubType="Atlas",
        objectKwargs={"Faction": "Bar", "Skill": 4},
    )
