...
class Camera:
    ...
    def collectImage(self, timeout_s = 1) -> np.ndarray:
...
class RTSPCamera(RemoteCamera):
    ...
    def rtsp_url(self) -> str:
        ...
        if self.auth is not None and len(self.auth) == 2:
            user, pw = self.auth
            return f"rtsp://{user}:{pw}@{self.address}"
        return f"rtsp://{self.address}"
...
class CaptureConfiguration:
    def __init__(self, path="nervousConfiguration.json"):
        self.loadConfiguration(path=path)

    def readConfigFile(self, path="nervousConfiguration.json"):
        try:
    ...
    def loadConfiguration(self, path="nervousConfiguration.json"):
        global cameras
        cameras = {}
        ...
        self.password = config.get("password", "gtdgdb")
        ...
        for camName, camDef in config.items():
            ...
            elif type(camDef) == dict and (str(camDef.get('addr', '')).startswith('rtsp://') or ':554' in str(camDef.get('addr', ''))):
                name = camName.replace("RTSPCamera", "")
                addr = camDef['addr']
                rot = camDef['rot']
                auth = camDef.get("auth", None)
                az = np.float32(json.loads(camDef['az']))
                metadata = {k: v for k, v in camDef.items() if k not in ['addr', 'rot', 'auth', 'az']}
                cameras[name] = RTSPCamera(address=addr, activeZone=az, camName=name, auth=auth, metadata=metadata)
            elif type(camDef) == dict and str(camName) not in ['rsc', 'calibrationPlan', 'hex']:
                addr = camDef['addr']
                rot = camDef['rot']
                auth = camDef.get("auth", None)
                az = np.float32(json.loads(camDef['az']))
                metadata = {k: v for k, v in camDef.items() if k not in ['addr', 'rot', 'auth', 'az']}
                cameras[camName] = RemoteCamera(address=addr, activeZone=az, camName=camName, auth=auth, metadata=metadata)
    ...
    def buildConfiguration(self):
        # Read the existing config to preserve non-camera settings
        try:
            old_config = self.readConfigFile()
        except Exception:
            old_config = {}

        # Merge our active cameras into the config
        for camName, cam in self.cameras.items():
            if isinstance(cam, RTSPCamera) and not camName.startswith("RTSPCamera"):
                config_name = f"RTSPCamera{camName}"
            elif isinstance(cam, RTSPCamera):
                config_name = camName  # Already has the prefix
            else:
                config_name = camName
            ...
            if hasattr(cam, 'metadata') and cam.metadata:
                for k, v in cam.metadata.items():
                    old_config[config_name][k] = v
            
        # Remove deleted cameras
        # We find keys in old_config that look like cameras but aren't in self.cameras
        keys_to_delete = []
        for key in old_config.keys():
            is_camera = False
            cam_name_no_prefix = key.replace("RTSPCamera", "")
            if key == "pov" or "RTSPCamera" in key or key not in ['rsc', 'calibrationPlan', 'hex']:
                # It looks like a camera key. Is it in our active cameras?
                if cam_name_no_prefix not in self.cameras and key not in self.cameras:
                    # Also make sure it's not a protected non-camera key
                    if key not in ['rsc', 'calibrationPlan', 'hex']:
        ...
        old_config["password"] = self.password

        return old_config

    def saveConfiguration(self, path="nervousConfiguration.json"):
...
if __name__ == "__main__":
    cc = CaptureConfiguration(path="../nervousConfiguration.json")
    start = time()
    for i in range(5):
        cc.capture()
    duration = time() - start
